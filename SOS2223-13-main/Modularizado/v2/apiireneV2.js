...
app.put('/api/v2/localentities/:city/:year', (req, res) => {
  ...
  /*
  if (isNaN(body.landline) || isNaN(body.president_appointment_date) || isNaN(body.surface_extension) 
  || isNaN(body.population) || isNaN(body.expense) || isNaN(body.income)
  || body.landline == "" || body.president_appointment_date == "" || body.surface_extension == ""
  ||body.population == "" || body.expense == "" || body.income == "") {
...
app.get(BASE_API_URL+"/graficoIFR", (request, response) =>{
  console.log("Grafica");
  db.find({},{_id: 0}).sort({province: 1, president_appointment_date: 1}).exec(function(err, lista){
      if(err){
